...
| D08 | Show Student lease | One process-local server grant may be active per teacher Show Student flow. Default TTL is 120 seconds; startup configuration `SHOW_STUDENT_TTL_SECONDS` may set 30–300 seconds and is validated fail-fast. The URL credential is a server-generated opaque value with at least 128 bits of CSPRNG entropy; it is never derived from student ID, alias, group ID, timestamp, or other predictable data, and contains no teacher-private identifier or data. The registry retains only a cryptographic verifier/hash, opaque grant ID/version, teacher session/owner binding, selected student and group binding, receipt fingerprint/non-secret idempotency metadata, redemption/revocation state, and expiry; no student data or recoverable secret is persisted. The viewer credential is short-lived, and exactly one active grant per teacher is sufficient; issuing a new grant invalidates the previous grant. Restart safely revokes all grants. |
...
`POST .../show-student` body is exactly `{studentId}`; TTL is server configuration, not teacher-selected business data. The first successful issuance is `201` and returns bootstrap secrets once. Same-key/same-payload replay is `200` with an explicit non-secret replay receipt (`expiresAt`, `replay:true`, and optionally the public `grantId`) and never returns `accessCode`, `accessUrl`, or `accessToken`. Same-key/different-payload is `409` without bootstrap secrets. A teacher that loses the first response must issue a new grant with a new idempotency key. `DELETE` returns `204`. Exchange accepts exactly one of `{token}` or `{code}`; malformed input is `422`, throttling `429`, and invalid/expired/redeemed input is generic `404`. Viewer `GET` returns `200 ShowStudentDto` or generic `404`; it never redirects to teacher sign-in.
