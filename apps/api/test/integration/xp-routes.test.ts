...
describe('private XP API contracts', () => {
  it('enforces auth, ownership-as-404, validation, replay/conflict, and opaque cursors', async () => {
    ...
    expect((await app.inject({ method: 'PUT', url: `/api/v1/academic-years/${year.json().id}/calendar`, headers: { origin, cookie }, payload: { timezone: 'UTC', terms: [{ code: 'T1', startsOn: '2026-09-01', endsOn: '2026-11-30' }, { code: 'T2', startsOn: '2026-12-01', endsOn: '2027-03-31' }, { code: 'T3', startsOn: '2027-04-01', endsOn: '2027-07-01' }], holidays: [], slots: [{ groupId: group.json().id, weekday: day, startsAt: '00:00', endsAt: '23:59' }] } })).statusCode).toBe(200);
    expect((await app.inject({ method: 'POST', url: `/api/v1/groups/${group.json().id}/real-class-sessions/start`, headers: { origin, cookie, 'idempotency-key': '00000000-0000-4000-8000-000000000003' }, payload: { academicYearId: year.json().id } })).statusCode).toBe(201);
