...
export const xpLevelGrantTransitions = sqliteTable('xp_level_grant_transitions', { id: text('id').primaryKey(), sequence: integer('sequence').notNull(), unlockId: text('unlock_id').notNull(), kind: text('kind').notNull(), sourceEventId: text('source_event_id'), sourceReversalId: text('source_reversal_id'), occurredAt: text('occurred_at').notNull() }, (table) => [uniqueIndex('uq_xp_grant_transition_sequence').on(table.sequence), uniqueIndex('uq_xp_grant_transition_unlock').on(table.unlockId).where(sql`${table.kind} = 'GRANT'`)]);
