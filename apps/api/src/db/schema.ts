...
export const assessmentContexts = sqliteTable('assessment_contexts', { id: text('id').primaryKey(), groupId: text('group_id').notNull().references(() => groups.id), name: text('name').notNull(), archivedAt: text('archived_at'), createdAt: text('created_at').notNull() });
