...
describe('process-local Show Student lease security', () => {
  it('creates opaque high-entropy credentials and deterministically replays exact creates', () => {
    const registry = new LeaseRegistry({ ttlSeconds: 120, now: () => new Date('2026-01-01T00:00:00.000Z') });
    const first = registry.create(input());
    const replay = registry.create(input());
    ...
    expect(registry.debugRecords()[0]).not.toMatchObject({ accessToken: first.accessToken, accessCode: first.accessCode });
  });
  ...
  it('rejects idempotency conflicts, cross-lease credentials, and guessed/replayed secrets', () => {
    ...
    const second = registry.create(input({ idempotencyKey: '00000000-0000-4000-8000-000000000002', studentId: 'student-2' }));
  ...
  it('enforces expiry boundaries, replacement/revoke events, and restart fail-closed', () => {
    ...
    const first = registry.create(input()); now = new Date('2026-01-01T00:00:30.000Z');
    ...
    const second = registry.create(input({ idempotencyKey: '00000000-0000-4000-8000-000000000002' })); registry.revoke(input());
  ...
  it('throttles code attempts per client and per teacher/grant in rolling ten-minute windows', () => {
    const registry = new LeaseRegistry({ ttlSeconds: 120 }); const created = registry.create(input());
  ...
  it('validates the configured TTL and receipt fingerprint replay/conflict', () => {
    ...
    const registry = new LeaseRegistry({ ttlSeconds: 120 }); const receipt = registry.create(input({ receiptFingerprint: 'fp' }));
    expect(registry.create(input({ receiptFingerprint: 'fp' }))).toEqual(receipt);
