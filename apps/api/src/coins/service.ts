...
const manualSources = ['PERSONAL_IMPROVEMENT','EXCEPTIONAL_FRENCH','EXCEPTIONAL_COLLABORATION','SPECIAL_CHALLENGE'] as const;
type ManualSource = typeof manualSources[number];
const now = () => new Date().toISOString();
const fingerprint = (value: unknown) => JSON.stringify(value, Object.keys(value as object).sort());
function optionalKey(key: string | undefined) { if (key === undefined) return undefined; if (!/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(key)) throw new ApiError('VALIDATION_FAILED',422,'A UUID v4 Idempotency-Key header is required.'); return key; }
function isManualCorrectionTargetRace(error: unknown) { return error instanceof Error && (error as { code?: string }).code === 'SQLITE_CONSTRAINT_UNIQUE' && error.message === 'UNIQUE constraint failed: coin_ledger.correction_of_id'; }
function isRedemptionRefundRace(error: unknown) { return error instanceof Error && (error as { code?: string }).code === 'SQLITE_CONSTRAINT_UNIQUE' && error.message === 'UNIQUE constraint failed: coin_ledger.redemption_id'; }
...
export function grantManual(db: Database.Database, teacherId: string, studentId: string, academicYearId: string, source: string, key?: string) {
  if (!manualSources.includes(source as ManualSource)) throw new ApiError('VALIDATION_FAILED',422,'The manual coin source is invalid.');
  const requestId = optionalKey(key);
  const requestFingerprint = repository.fingerprint({ operation: 'manual-coin-grant', studentId, academicYearId, source });
  if (requestId) {
    const existing = repository.findByRequest(db, teacherId, requestId);
    if (existing) {
      ...
      return { id: existing.id, studentId: existing.studentId, academicYearId: existing.academicYearId, replay: true };
    }
  }
  ...
  try {
    const id = db.transaction(() => repository.grant(db,{studentId,academicYearId,source,ownerTeacherId:teacherId,clientRequestId:requestId,requestFingerprint: requestId ? requestFingerprint : undefined}))();
    return { id, studentId, academicYearId, replay: false };
  } catch (error) {
...
export function reverseManualGrant(db: Database.Database, teacherId: string, grantId: string, key?: string) {
  const requestId = optionalKey(key);
  const requestFingerprint = repository.fingerprint({ operation: 'manual-coin-correction', grantId });
  if (requestId) {
    const existing = repository.findByRequest(db, teacherId, requestId);
    if (existing) {
      if (existing.requestFingerprint !== requestFingerprint || existing.source !== 'MANUAL_CORRECTION' || existing.correctionOfId !== grantId) throw new ApiError('CONFLICT',409,'Idempotency-Key was already used for a different request.');
      return { id: existing.id, grantId, studentId: existing.studentId, academicYearId: existing.academicYearId, source: existing.source, amount: existing.amount, replay: true };
    }
  }
  const grant = repository.findLedger(db, grantId, teacherId);
  if (!grant) throw new ApiError('NOT_FOUND',404,'Coin grant not found.');
  if (grant.studentArchivedAt || grant.yearArchivedAt) throw new ApiError('VALIDATION_FAILED',422,'Archived students and academic years are read-only.');
  if (!manualSources.includes(grant.source as ManualSource) || grant.amount !== 1 || grant.correctionOfId !== null || grant.redemptionId !== null) throw new ApiError('CONFLICT',409,'Only unallocated manual coin grants can be corrected.');
  if (repository.manualCorrectionForGrant(db, grantId)) throw new ApiError('CONFLICT',409,'Coin grant is already corrected.');
  if (repository.activeRedemptionForGrant(db, grantId)) throw new ApiError('CONFLICT',409,'Allocated assessment coin grants cannot be corrected.');
  try {
    return db.transaction(() => {
      if (requestId) {
        ...
        if (existing) {
          if (existing.requestFingerprint !== requestFingerprint || existing.source !== 'MANUAL_CORRECTION' || existing.correctionOfId !== grantId) throw new ApiError('CONFLICT',409,'Idempotency-Key was already used for a different request.');
          return { id: existing.id, grantId, studentId: existing.studentId, academicYearId: existing.academicYearId, source: existing.source, amount: existing.amount, replay: true as const };
        }
      }
      if (repository.manualCorrectionForGrant(db, grantId)) throw new ApiError('CONFLICT',409,'Coin grant is already corrected.');
      const id = repository.correctManualGrant(db, { studentId: grant.studentId, academicYearId: grant.academicYearId, grantId, ownerTeacherId: teacherId, clientRequestId: requestId, requestFingerprint: requestId ? requestFingerprint : undefined });
      return { id, grantId, studentId: grant.studentId, academicYearId: grant.academicYearId, source: 'MANUAL_CORRECTION' as const, amount: -1 as const, replay: false as const };
    })();
  } catch (error) {
    if (!isManualCorrectionTargetRace(error)) throw error;
    if (requestId) {
      const existing = repository.findByRequest(db, teacherId, requestId);
      if (existing) {
        if (existing.requestFingerprint !== requestFingerprint || existing.source !== 'MANUAL_CORRECTION' || existing.correctionOfId !== grantId) throw new ApiError('CONFLICT',409,'Idempotency-Key was already used for a different request.');
    ...
    throw new ApiError('CONFLICT',409,'Coin grant is already corrected.');
  }
}
export function spend(db: Database.Database, teacherId: string, studentId: string, contextId: string, rewardId: string, requestKey?: string) {
  ...
  if(!requestKey || !/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(requestKey)) throw new ApiError('VALIDATION_FAILED',422,'A UUID v4 Idempotency-Key header is required.');
...
export function reverse(db: Database.Database, teacherId: string, redemptionId: string) {
  const row=db.prepare(`SELECT r.*,s.group_id AS groupId,g.owner_teacher_id AS ownerTeacherId,g.academic_year_id AS academicYearId FROM advantage_redemptions r JOIN students s ON s.id=r.student_id JOIN groups g ON g.id=s.group_id WHERE r.id=? AND g.owner_teacher_id=?`).get(redemptionId,teacherId) as any;
  if(!row) throw new ApiError('NOT_FOUND',404,'Redemption not found.');
  if(row.reversed_at) throw new ApiError('CONFLICT',409,'Redemption is already reversed.');
  try {
    return db.transaction(() => {
      const refund=repository.refund(db,{studentId:row.student_id,academicYearId:row.academicYearId,amount:row.cost,redemptionId});
      repository.releaseAllocations(db,redemptionId);
      const reversedAt=now();
      db.prepare('UPDATE advantage_redemptions SET reversal_ledger_id=?,reversed_at=? WHERE id=?').run(refund,reversedAt,redemptionId);
      return {redemptionId,rewardId:row.reward_id,cost:row.cost,trigger:'MANUAL',refundLedgerEntryId:refund,reversedAt};
    })();
  } catch (error) {
    if (isRedemptionRefundRace(error)) throw new ApiError('CONFLICT',409,'Redemption is already reversed.');
    throw error;
  }
}
