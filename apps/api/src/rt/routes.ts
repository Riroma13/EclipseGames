...
export function registerRtRoutes(app: FastifyInstance, db: Database.Database) {
  ...
  app.post('/api/v1/real-class-sessions/:sessionId/rt-entries', { preHandler: [session, validateBody(body)] }, async (request, reply) => {
    ...
    const result = service.upsertEntries(db, teacher(request), uuidSchema.parse(params(request).sessionId), body.parse(request.body).entries, key);
