...
import { levelForXp, progressForXp, levelThresholds } from '@eclipse/domain';
...
export type EventRow = { id:string; studentId:string; academicYearId:string; category:XpCategory; baseXp:number; specialtyBonusXp:number; effectiveXp:number; specialtyAtAward:string|null; comment:string|null; realClassSessionId:string|null; termId:string|null; createdAt:string; reversedAt:string|null };
const eventSelect = `SELECT e.id, e.student_id AS studentId, e.academic_year_id AS academicYearId, e.category, e.base_xp AS baseXp, e.specialty_bonus_xp AS specialtyBonusXp, e.effective_xp AS effectiveXp, e.specialty_at_award AS specialtyAtAward, e.comment, e.real_class_session_id AS realClassSessionId, e.term_id AS termId, e.created_at AS createdAt, r.created_at AS reversedAt FROM xp_evidence_events e LEFT JOIN xp_evidence_reversals r ON r.target_event_id=e.id`;
...
function transitions(db: Database.Database, studentId: string, academicYearId: string, before: number, after: number, sourceEventId: string, sourceReversalId: string|null) {
  const ids: string[] = [];
  for (let level = 2; level <= 8; level += 1) { const threshold = levelThresholds[level - 1]; const row = db.prepare(`SELECT * FROM xp_level_unlocks WHERE student_id=? AND academic_year_id=? AND level=?`).get(studentId,academicYearId,level) as any; if (before < threshold && after >= threshold) { if (!row) { const unlockId=randomUUID(); db.prepare(`INSERT INTO xp_level_unlocks (id,student_id,academic_year_id,level,active,first_crossed_at,first_source_event_id,updated_at) VALUES (?,?,?,?,?,?,?,?)`).run(unlockId,studentId,academicYearId,level,1,now(),sourceEventId,now()); const id=randomUUID(); db.prepare(`INSERT INTO xp_level_grant_transitions (id,sequence,unlock_id,kind,source_event_id,source_reversal_id,occurred_at) VALUES (?,COALESCE((SELECT MAX(sequence)+1 FROM xp_level_grant_transitions),1),?,'GRANT',?,NULL,?)`).run(id,unlockId,sourceEventId,now()); ids.push(id); } else if (!row.active) { db.prepare(`UPDATE xp_level_unlocks SET active=1,updated_at=? WHERE id=?`).run(now(),row.id); const id=randomUUID(); db.prepare(`INSERT INTO xp_level_grant_transitions (id,sequence,unlock_id,kind,source_event_id,source_reversal_id,occurred_at) VALUES (?,COALESCE((SELECT MAX(sequence)+1 FROM xp_level_grant_transitions),1),?,'REINSTATE',?,NULL,?)`).run(id,row.id,sourceEventId,now()); ids.push(id); } } else if (before >= threshold && after < threshold && row?.active) { db.prepare(`UPDATE xp_level_unlocks SET active=0,updated_at=? WHERE id=?`).run(now(),row.id); const id=randomUUID(); db.prepare(`INSERT INTO xp_level_grant_transitions (id,sequence,unlock_id,kind,source_event_id,source_reversal_id,occurred_at) VALUES (?,COALESCE((SELECT MAX(sequence)+1 FROM xp_level_grant_transitions),1),?,'REVOKE',NULL,?,?)`).run(id,row.id,sourceReversalId,now()); ids.push(id); } }
  return ids;
}
export function createEvent(db: Database.Database, input: any) { const before = derive(db,input.studentId,input.academicYearId).annualEffectiveXp; db.prepare(`INSERT INTO xp_evidence_events (id,owner_teacher_id,student_id,academic_year_id,category,base_xp,real_class_session_id,term_id,specialty_at_award,specialty_category_at_award,bonus_eligible_at_award,specialty_bonus_xp,effective_xp,comment,created_at,created_by_teacher_id,client_request_id,request_fingerprint) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(input.id,input.teacherId,input.studentId,input.academicYearId,input.category,input.baseXp,input.realClassSessionId,input.termId,input.specialtyAtAward,input.specialtyCategoryAtAward,input.bonusEligibleAtAward?1:0,input.specialtyBonusXp,input.effectiveXp,input.comment,input.createdAt,input.teacherId,input.requestId,input.fingerprint); const after=derive(db,input.studentId,input.academicYearId).annualEffectiveXp; return { event: findEvent(db,input.teacherId,input.id)!, transitionIds: transitions(db,input.studentId,input.academicYearId,before,after,input.id,null) }; }
export function reverseEvent(db: Database.Database, input: any) { const event=findEvent(db,input.teacherId,input.eventId); if (!event) throw new ApiError('NOT_FOUND',404,'XP evidence not found.'); if (event.reversedAt) throw new ApiError('VALIDATION_FAILED',422,'XP evidence is already reversed.'); const before=derive(db,event.studentId,event.academicYearId).annualEffectiveXp; db.prepare(`INSERT INTO xp_evidence_reversals (id,owner_teacher_id,target_event_id,reason,created_at,created_by_teacher_id,client_request_id,request_fingerprint) VALUES (?,?,?,?,?,?,?,?)`).run(input.id,input.teacherId,event.id,input.reason,input.createdAt,input.teacherId,input.requestId,input.fingerprint); const after=derive(db,event.studentId,event.academicYearId).annualEffectiveXp; return { reversal: { id:input.id,targetEventId:event.id,reason:input.reason,createdAt:input.createdAt,studentId:event.studentId,academicYearId:event.academicYearId }, transitionIds: transitions(db,event.studentId,event.academicYearId,before,after,event.id,input.id) }; }
