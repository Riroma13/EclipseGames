...
it('exposes ordered immutable transitions for SPEC-0005 replay', () => {
  ...
  const t='00000000-0000-4000-8000-000000000101', y='00000000-0000-4000-8000-000000000102', g='00000000-0000-4000-8000-000000000103', s='00000000-0000-4000-8000-000000000104';
   db.prepare('INSERT INTO teacher_accounts VALUES (?,?,?,?)').run(t,'t@test','h','now'); db.prepare('INSERT INTO academic_years (id,owner_teacher_id,label,starts_on,ends_on,created_at) VALUES (?,?,?,?,?,?)').run(y,t,'Y','2026','2027','now'); db.prepare('INSERT INTO groups VALUES (?,?,?,?,?)').run(g,t,y,'G','now'); db.prepare('INSERT INTO students (id,group_id,real_name,alias,avatar,specialty,created_at) VALUES (?,?,?,?,?,?,?)').run(s,g,'Student','S','default','Leader','now'); startFixtureRealClassSession(db,{teacher:t,year:y,group:g,key:'00000000-0000-4000-8000-000000000199'});
...
function fixture() {
  ...
  const t='00000000-0000-4000-8000-000000000201', y='00000000-0000-4000-8000-000000000202', g='00000000-0000-4000-8000-000000000203', s='00000000-0000-4000-8000-000000000204';
   db.prepare('INSERT INTO teacher_accounts VALUES (?,?,?,?)').run(t,'t201@test','h','now'); db.prepare('INSERT INTO academic_years (id,owner_teacher_id,label,starts_on,ends_on,created_at) VALUES (?,?,?,?,?,?)').run(y,t,'Y','2026','2027','now'); db.prepare('INSERT INTO groups VALUES (?,?,?,?,?)').run(g,t,y,'G','now'); db.prepare('INSERT INTO students (id,group_id,real_name,alias,avatar,specialty,created_at) VALUES (?,?,?,?,?,?,?)').run(s,g,'Student','S','default','Leader','now'); startFixtureRealClassSession(db,{teacher:t,year:y,group:g,key:'00000000-0000-4000-8000-000000000299'});
  return {db,t,y,s};
}
