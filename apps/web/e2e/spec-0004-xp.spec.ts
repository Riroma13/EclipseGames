...
test('SPEC-0004 private XP flow registers authoritative bonus and exposes correction only in teacher workspace', async ({ page }) => {
  ...
  await expect(page.getByText('XP base +3 · XP efectivo +4')).toBeVisible();
   await expect(page.locator('.student-progress-row')).toContainText('4 XP'); await expect(page.locator('.undo-banner').getByRole('button')).toBeVisible();
...
test('XP quick actions show specialty totals while preserving canonical base requests and Undo', async ({ page }) => {
  ...
  const options = page.locator('.xp-values > button:not(:last-child)');
  await page.getByRole('button', { name: /Communication Specialist/ }).click();
  const communicationCategory = () => page.locator('button[aria-label^="COMMUNICATION"]');
  const precisionCategory = () => page.locator('button[aria-label^="PRECISION"]');
  await expect(communicationCategory()).toContainText('Voice & expression');
  await expect(communicationCategory()).toHaveAccessibleName('COMMUNICATION, bonus de especialidad +1');
  await expect(precisionCategory()).not.toContainText('bonus de especialidad +1');
  await communicationCategory().click();
  await expect(options).toHaveText(['+1', '+2', '+3']);
  await expect(page.locator('.xp-values button').nth(0)).toHaveAccessibleName('+1');
  ...
  await expect(precisionCategory()).toContainText('Care & accuracy');
  await expect(precisionCategory()).toHaveAccessibleName('PRECISION, bonus de especialidad +1');
  await expect(communicationCategory()).not.toContainText('bonus de especialidad +1');
  await communicationCategory().click();
  await expect(options).toHaveText(['+1', '+2', '+3']);
  await expect(page.locator('.xp-values button').nth(0)).toHaveAccessibleName('+1');
   await page.getByRole('button', { name: 'Cancelar', exact: true }).click();
  ...
  await expect(page.locator('.feedback')).toHaveText('XP base +1 · XP efectivo +2');
  await expect(page.locator('.student-progress-row')).toContainText('2 XP');
  await precisionCategory.click();
  await options.nth(1).click();
  await expect(page.locator('.feedback')).toHaveText('XP base +2 · XP efectivo +3');
  await expect(page.locator('.student-progress-row')).toContainText('5 XP');
  await precisionCategory.click();
  await options.nth(2).click();
  await expect(page.locator('.feedback')).toHaveText('XP base +3 · XP efectivo +4');
  await expect(page.locator('.student-progress-row')).toContainText('9 XP');
   await expect(page.locator('.activity-summary')).toContainText('Base +3 · Bonus +1 · Effective +4');
  ...
  await expect(page.locator('.student-progress-row')).toContainText('5 XP');
   await expect(page.locator('.activity-summary')).toContainText('Reversed');
});

test('specialty sheets share discipline glyphs and matching Quick XP treatment', async ({ page }) => {
  const specialties = [
    { realName: 'Leader Student', alias: 'Leader', specialty: 'Leader', discipline: 'COMMUNICATION', glyph: '◒', effectiveValues: ['+1', '+2', '+3'] },
    { realName: 'Strategist Student', alias: 'Strategist', specialty: 'Strategist', discipline: 'PRECISION', glyph: '⌖', effectiveValues: ['+1', '+2', '+3'] },
    { realName: 'Disciplined Student', alias: 'Disciplined', specialty: 'Disciplined', discipline: 'CONSISTENCY', glyph: '↗︎', effectiveValues: ['+1', '+2', '+3'] },
    { realName: 'Helper Student', alias: 'Helper', specialty: 'Helper', discipline: 'COLLABORATION', glyph: '∞', effectiveValues: ['+1', '+2', '+3'] },
  ];
  const login = await page.request.post('/api/v1/auth/session', { data: { email: 'teacher@example.test', password: 'change-me-in-development' } });
  expect(login.status()).toBe(204);
  const cookie = login.headers()['set-cookie']?.split(';')[0];
  ...
  for (const entry of specialties) {
    ...
    const matching = panel.locator(`button[aria-label^="${entry.discipline}"]`);
    await expect(matching).toHaveAccessibleName(new RegExp(`^${entry.discipline}, bonus de especialidad \\+1$`));

    await matching.click();
    await expect(panel.locator('.xp-values > button:not(:last-child)')).toHaveText(entry.effectiveValues);
  }
});
...
test('stale XP completion cannot publish into a newly selected student or clear its pending request', async ({ page }) => {
  ...
  await expect(page.locator('.feedback')).toHaveText('XP base +1 · XP efectivo +1');
...
test('stale Undo completion cannot publish into a newly selected student', async ({ page }) => {
  ...
  await expect(page.getByText('XP base +1 · XP efectivo +1')).toBeVisible();
  await expect(page.getByRole('button', { name: 'Undo', exact: true })).toBeVisible();
  ...
  await expect(studentBPanel.locator('.student-progress-row')).toContainText('0 XP');
   const studentBActivity = page.locator('.activity-summary');
  ...
  const studentBFeedbackCount = await studentBPanel.locator('.feedback').count();
   const studentBUndoCount = await studentBPanel.locator('.undo-banner').getByRole('button').count();
  ...
  expect(await studentBPanel.locator('.feedback').count()).toBe(studentBFeedbackCount);
   expect(await studentBPanel.locator('.undo-banner').getByRole('button').count()).toBe(studentBUndoCount);
  ...
  await page.getByRole('button', { name: /Student A/ }).click();
  await expect(page.getByRole('heading', { name: 'Student A' })).toBeVisible();
  await expect(page.locator('.student-panel .student-progress-row')).toContainText('0 XP');
   await expect(page.locator('.activity-summary')).toContainText('Reversed');
});
